//#region node_modules/media-played-ranges-mixin/dist/index.js
function MediaPlayedRangesMixin(Base) {
	return class MediaPlayedRanges extends Base {
		...
		onPlaybackStart(param = {}) {
			...
			if (!this._currentPlayedRange) this._currentPlayedRange = {
				start: t,
				end: t
			};
		}
		...
		_commitCurrentRange(time) {
			...
			if (typeof time === "number") this._currentPlayedRange.end = time;
		...
		addPlayedRange(start, end) {
			...
			const allRanges = [...this._playedRanges, {
				start,
				end
			}];
		...
		get played() {
			...
			if (!this.paused && !this._currentPlayedRange && typeof time === "number") this._currentPlayedRange = {
				start: time,
				end: time
			};
			if (this._currentPlayedRange && typeof time === "number") {
				if (time > this._currentPlayedRange.end) this._currentPlayedRange.end = time;
				this.addPlayedRange(this._currentPlayedRange.start, this._currentPlayedRange.end);
			}
			if (!this._playedRanges.length) return createTimeRangesObj([[0, 0]]);
...
//#endregion
export { MediaPlayedRangesMixin as t };

//# sourceMappingURL=dist-aEENv_V1.js.map